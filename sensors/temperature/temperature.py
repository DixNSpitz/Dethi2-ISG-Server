...
sense_temp = SenseTemperature(1, 0x44, 0x2C, [0x06])  # Initialization

for i in range(5):  # Loop for reading the values
    cTemp, fTemp, humidity = sense_temp.read()
    print(f"Temperature in Celsius is : {cTemp:.2f} C")
    print(f"Temperature in Fahrenheit is : {fTemp:.2f} F")
    print(f"Relative Humidity is : {humidity:.2f} %RH")
